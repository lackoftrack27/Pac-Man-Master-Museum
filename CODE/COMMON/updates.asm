...
;   JUMP IF AT POWER DOT
    ...
    LD A, (mazeMutatedTbl)      ; STARTING TILE ID OF MUTATED DOT TILES
    LD B, A
    LD A, (HL)
    CP A, B
    JP NC, @mutatedTile ; IF SO, DOT IS CONTAINED IN A MUTATED TILE. NEED TO USE A DIFFERENT TABLE
;
;   NORMAL DOT PROCESSING (NORMAL TILE)
;
    ...
    JP @updateCollision
;
;   NORMAL DOT/MUTATED DOT PROCESSING (TILE HAS AT LEAST 1 MUTATED DOT)
;
@mutatedTile:
;   NORMAL OR MUTATED DOT HAS BEEN EATEN (ALL WE KNOW AT THIS POINT IS THAT TILE ID IS IN MUTATED RANGE)
    INC HL
...
+:
    ...
    LD A, (HL)              ; GET VALUE AT OFFSET
    LD (tileBuffer + 1), A  ; STORE AS LOW BYTE
    ; SET FLIP BITS IN BUFFER
    INC L
    LD B, (HL)
    RES 7, B    ; REMOVE MUTATED DOT FLAG FROM TABLE'S FLIP BYTE
    LD A, (DE) 
    XOR A, B    ; XOR WITH CURRENT FLIP BITS
    LD (tileBuffer + 2), A  ; STORE AS HIGH BYTE
    ; NORMAL/MUTATED DOT FINISH
...
;   A POWER DOT HAS BEEN EATEN
    ...
    ; GET BACK POWER DOT TABLE POINTER
    POP HL
    ; PREPARE FOR LOOP
    LD DE, tileBuffer
...
+:
    ...
    ; REPLACE TILE
    INC DE
    EX DE, HL   ; DE: MUTATED DOT TABLE PTR, HL: RAM TILEMAP PTR
    LD A, (DE)
    AND A, $07  ; REMOVE MUTATED DOT FLAG FROM TABLE FLIP BYTE
    XOR A, (HL) ; XOR WITH FLIP FLAGS OF CURRENT TILE
    EX DE, HL   ; DE: RAM TILEMAP PTR, HL: MUTATED DOT TABLE PTR
    LD (DE), A  ; STORE AS HIGH BYTE
    DEC DE
    DEC L
    LD A, (HL)
