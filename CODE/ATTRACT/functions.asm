...
*/
getPressedInputs:
;   GET ADDRESS OF CONTROLLER 1'S DATA (UP,DOWN,LEFT,RIGHT,B,C)
    LD HL, controlPort1
...
/*
    INFO: SAME THING AS ABOVE, BUT FOR MD BUTTONS (A, START)
    INPUT: NONE
    OUTPUT: NONE
    USES: AF, HL
*/
getPressedInputsMD:
;   GET ADDRESS OF CONTROLLER 1'S DATA (A, START)
    LD HL, controlPort1MD
;   GET DIFFERENCE BETWEEN PREVIOUS INPUT
    LD A, (prevInputMD)
    CPL
;   BUTTON IS ONLY CONSIDERED PRESSED IF THERE WAS DIFFERENCE BETWEEN INPUTS AND BUTTON IS PRESSED NOW
    AND A, (HL)
    LD (pressedButtonsMD), A
;   INPUT IS NOW PREVIOUS INPUT
    LD A, (HL)
    LD (prevInputMD), A
    RET
